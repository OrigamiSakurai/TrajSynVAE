...
def Overlapping_ratio_count_prepare(location, traj):
...
def Overlapping_ratio_count(location, target, trajs):
    ...
    for traj in trajs:
        ...
        Traj, _ = Overlapping_ratio_count_prepare(loc, trajs[traj])
        A = 48
        Overlapping_ration.append(sum([1 for x in range(A) if Target[x] == Traj[x]]))
    Overlapping_ration = sorted(Overlapping_ration, reverse=True)
    return Overlapping_ration
'''
...
def Overlapping_ratio_plot(method, original):
    ...
    for user in method:
        ...
        for traj in method[user]:
            ...
            trajs = original[user]
            ration = Overlapping_ratio_count(location, target, trajs)
            if len(ration) < 3:
                zeros = np.zeros(3 - len(ration))
                ration = np.concatenate((ration, zeros))
            top[1].append(ration[0])
            top[2].append(ration[1])
            top[3].append(ration[2])
            k += 1
    ...
    plt.tight_layout()
    plt.savefig(param.save_path + '/plots' + '/Overlapping_ration.png')
'''

# Draw the Traj of specific user for further analysis
