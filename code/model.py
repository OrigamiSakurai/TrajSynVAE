...
class ABS_TIM_EMB(nn.Module):
    ...
    def forward(self, x):
        a = torch.div(torch.arange(0.0, self.d_model).to(device), 2, rounding_mode='floor') * 2
...
class USR_EMB(nn.Module):
    ...
    def forward(self, x):
        usr2id = torch.tensor(np.array([[np.where(self.USERLIST == u.item())[0][0] for u in x[user]] for user in range(len(x))])).long().to(device)
...
class POI_EMB(nn.Module):
    ...
    def forward(self, x):
        return torch.tensor(np.array([self.POI[np.ix_(locs.cpu())] for locs in x]), dtype=torch.float).to(device)
...
class DECODER(nn.Module):
    ...
    def forward(self, usr, pos, z):#, loc_weight, usr_weight):
        ...
        h1 = torch.zeros(1, batchsize, self.decoder_rnn_hidden_size).to(device)
        c1 = torch.zeros(1, batchsize, self.decoder_rnn_hidden_size).to(device)
...
class VAE(nn.Module):
    ...
    def loss(step, mean, std, lout, tout, inseq):
        ...
        # LL_L_weight = torch.tensor((1 + np.exp(-1 * (step - 1e3)))).to(device)
        LOSS = LL_T + LL_L + KL
    ...
    def sample(self, lout, tout, last_loc=-1):
        ...
        if last_loc >= 0:
            prob[last_loc] = 0
            # prob[np.where(self.VALIDID == last_loc)] = 0
    ...
    def inference(self, user):
        ...
        # Decoder & Generate
        lout1, tout1 = self.decoder(usr[0].view(1,1), torch.zeros((1,1), dtype=torch.float).to(device), z[0].view(1,1,-1))
        l, t = self.sample(lout1, tout1)
        X = {'loc': [l], 'tim': [0], 'sta': [t]}
        last_location = int(X['loc'][-1])

        for i in range(1, 5000):
            time = torch.tensor([X['tim']], dtype=torch.float).to(device)
            louti, touti = self.decoder(usr[:i].unsqueeze(0), time, z[:i].unsqueeze(0))
            l, t = self.sample(louti, touti, last_location)
            X['tim'].append(X['tim'][-1] + X['sta'][-1])
            X['loc'].append(l)
            X['sta'].append(t)
            if X['tim'][-1] + X['sta'][-1] >= self.infer_maxlast:
                break  
            last_location = int(X['loc'][-1])

        X['tim'] = np.array(X['tim'])
    ...
    def train(self, trainset, validset, testset = None):
        ...
        for epoch in range(self.epoches):
            ...
            for idx, bat in enumerate(mydataloader):
                ...
                # Input
                loc = bat['loc'].clone().detach().long().to(device)
                tim = bat['sta'].clone().detach().long().to(device)
                usr = bat['usr'].clone().detach().long().to(device)
                pos = bat['tim'].clone().detach().float().to(device)
